  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-extrabold font-display text-slate-855 dark:text-white leading-tight">Team Goals Tracking</h2>
          <p className="text-xs text-slate-500">Establish operational milestones, assign performance targets, and track milestones progress.</p>
        </div>
        <Button 
      ...
      <Card className="accent-border-mint">
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            {activeReport && (
              <span className="text-xs text-slate-450 font-bold shrink-0 font-mono">
      ...
      <Card className="accent-border-mint">
        ...
        <CardContent>
          {isGoalsLoading ? (
            <div className="text-center py-12 text-slate-400 text-xs">Loading employee targets...</div>
          ) : goals.length > 0 ? (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {goals.map((g) => (
                  <TableRow key={g.id}>
                    <TableCell className="font-bold text-slate-805 dark:text-white">{g.title}</TableCell>
                    <TableCell className="text-slate-655 dark:text-slate-400 font-semibold">{g.description || 'No description set'}</TableCell>
                    <TableCell className="font-mono text-slate-700 dark:text-slate-450">{new Date(g.targetDate).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}</TableCell>
                    <TableCell>
                      <Badge 
                        variant={
            ...
            <div className="text-center py-12 text-slate-400 text-xs">No performance milestones assigned yet.</div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog
        ...
        footer={
          <>
            ...
            <Button onClick={handleSubmit} loading={createGoalMutation.isPending}>Confirm Assign</Button>
          </>
        }
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase">Description / Deliverables</label>
            <textarea 
              value={description} 
              onChange={(e) => setDescription(e.target.value)} 
              placeholder="Reduce query latency times down to 100ms on index page queries..."
              className="w-full bg-slate-50 border border-slate-100 rounded p-2.5 text-xs h-24 outline-none focus:border-amber-500 dark:bg-slate-800 dark:border-slate-700 dark:text-white"
            />
          </div>

          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase">Target Due Date *</label>
