      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative">
          ...
          >
            <Bell size={18} />
            {pendingLeaveCount > 0 && (
              <span className="absolute top-1.5 right-1.5 w-3.5 h-3.5 bg-rose-500 rounded-full text-[8px] font-bold text-white flex items-center justify-center animate-pulse">
                {pendingLeaveCount}
              </span>
            )}
          </button>
            ...
            <>
              ...
              <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl py-3 z-50 animate-fadeIn">
                <div className="px-4 pb-2 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-450">Notifications</span>
                  <span className="text-[9px] bg-rose-100 text-rose-700 px-1.5 py-0.5 rounded font-extrabold dark:bg-rose-950/40 dark:text-rose-400">{pendingLeaveCount} pending</span>
                </div>
                <div className="max-h-60 overflow-y-auto pt-1">
                  {pendingLeaveCount > 0 ? (
                    <div className="p-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 flex items-start gap-3 cursor-pointer">
                      <div className="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center text-amber-700 dark:bg-amber-950/40 dark:text-amber-400 shrink-0">
                        <ShieldAlert size={14} />
                      </div>
                      <div>
                        <span className="text-[11px] font-bold text-slate-800 dark:text-white block">HR Leave Request Approvals</span>
                        <p className="text-[10px] text-slate-500 mt-0.5">Jessica Day submitted a summer vacation leave request. Awaiting Admin signoff.</p>
                      </div>
                    </div>
                  ) : (
                    <div className="p-4 text-center text-slate-400 text-xs font-medium">All system logs clear. No pending alerts.</div>
                  )}
                </div>
              </div>
            </>
        ...
        <div className="flex items-center gap-3 border-l border-slate-100 pl-6 dark:border-slate-800">
          <div className="text-right">
            ...
            <span className="text-[9px] font-extrabold tracking-widest text-amber-700 dark:text-amber-500 uppercase">
              {session.role}
            </span>
          </div>
