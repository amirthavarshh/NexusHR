...
export const Settings: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Title */}
      <div>
        <h2 className="text-2xl font-extrabold font-display text-slate-855 dark:text-white leading-tight">Workspace Config Settings</h2>
        <p className="text-xs text-slate-500">Configure global organizational variables, security policies, and reset defaults.</p>
      </div>

      {/* Global Status Banner */}
      {statusMessage && (
        <div className="p-3 rounded text-xs font-bold bg-emerald-50 text-emerald-705 border border-emerald-250 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/30 animate-fadeIn">
          {statusMessage}
        </div>
      )}
          ...
          <CardContent className="space-y-4 pt-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Session Expiry Timeout (Minutes)</label>
                <Input value={sessionTimeout} onChange={(e) => setSessionTimeout(e.target.value)} type="number" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Maximum Failed Login Retries</label>
                <Input value={maxLoginRetries} onChange={(e) => setMaxLoginRetries(e.target.value)} type="number" />
              </div>
            </div>

            <div className="space-y-1.5 pt-2">
              <label className="text-[10px] font-bold text-slate-500 uppercase block">Enforce Multi-Factor Authentication (MFA)</label>
        ...
        <div className="space-y-6">
          <Card className="accent-border-lavender">
            ...
            <CardContent className="space-y-4 pt-4 text-xs">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Default Workspace Timezone</label>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Default Leaves Allocation (Annual Days)</label>
                <Input value={defaultLeavesAllocated} onChange={(e) => setDefaultLeavesAllocated(e.target.value)} type="number" />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">System Audit Logging Level</label>
