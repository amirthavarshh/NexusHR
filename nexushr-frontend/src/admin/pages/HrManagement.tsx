  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-extrabold font-display text-slate-855 dark:text-white leading-tight">HR Personnel Management</h2>
          <p className="text-xs text-slate-500">Create, monitor, and configure system permissions for HR Specialists and Officers.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <Card className={`accent-border-mint ${detailHR ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
          <CardHeader>
          ...
          <CardContent>
            {isLoading ? (
              <div className="text-center py-12 text-slate-400 text-xs">Loading HR roster...</div>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredHRs.map((hr) => (
                    <TableRow 
                      key={hr.id} 
                      className={`cursor-pointer ${detailHR?.id === hr.id ? 'bg-amber-50/30 dark:bg-amber-950/10' : ''}`}
                      onClick={() => setDetailHR(hr)}
                    >
                      <TableCell className="font-bold text-slate-805 dark:text-white">
                        <div>
                          <span>{hr.firstName} {hr.lastName}</span>
                          <span className="text-[10px] text-slate-400 block font-normal mt-0.5">{hr.email}</span>
                        </div>
                      </TableCell>
                      <TableCell className="text-slate-655 dark:text-slate-400 font-semibold">{hr.position}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex gap-2 justify-end">
                          <Button size="sm" variant="ghost" onClick={() => setDetailHR(hr)}>
                            <Eye size={12} className="text-slate-450" />
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => handleOpenEdit(hr)}>
                            <Edit size={12} className="text-slate-450" />
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => handleDelete(hr.id)}>
              ...
              <div className="text-center py-12 text-slate-400 text-xs">No HR profiles found matching criteria.</div>
            )}
          </CardContent>
        </Card>
          ...
          <Card className="accent-border-lavender animate-fadeIn">
            ...
            <CardContent className="pt-4 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-sm font-extrabold text-slate-855 dark:text-white block">{detailHR.firstName} {detailHR.lastName}</span>
                  <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider dark:bg-slate-800 dark:text-slate-350">{detailHR.position}</span>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-450 font-bold">System Credentials:</span>
                  <Badge variant="info">Granted</Badge>
                </div>
                <div className="bg-slate-50 p-2.5 rounded border border-slate-100 text-[10px] space-y-1.5 dark:bg-slate-800/20 dark:border-slate-800">
                  <div className="flex justify-between text-slate-500">
                    <span>Role Group:</span>
                    <span className="font-bold text-slate-700 dark:text-slate-300">ROLE_HR</span>
                  </div>
                  <div className="flex justify-between text-slate-500">
                    <span>Username Ref:</span>
                    <span className="font-mono text-slate-750 dark:text-slate-300">{detailHR.user?.username || detailHR.email.split('@')[0]}</span>
                  </div>
                </div>
              </div>

              <div className="pt-2 flex gap-2">
                <Button size="sm" variant="outline" className="flex-1" onClick={() => handleOpenEdit(detailHR)}>
                  <Edit size={12} />
                  <span>Edit details</span>
                </Button>
                <Button size="sm" variant="destructive" className="flex-1" onClick={() => handleDelete(detailHR.id)}>
                  <Trash2 size={12} />
                  <span>Terminate</span>
                </Button>
              </div>
      ...
      <Dialog 
        ...
        footer={
          <>
            ...
            <Button onClick={handleSubmit} loading={createHRMutation.isPending || updateHRMutation.isPending}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <Input value={department} disabled className="bg-slate-50 dark:bg-slate-800" />
            </div>
            <div className="space-y-1">
          ...
          {!editingHR && (
            <div className="border-t border-slate-100 dark:border-slate-800 pt-4 space-y-4">
              <span className="text-[10px] font-extrabold text-slate-450 block uppercase tracking-wide">System Login Credentials</span>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
