...
export const Dashboard: React.FC = () => {
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'DEPT_ASSIGNED':
      case 'LEAVE_APPROVED':
        return <FileCheck size={14} className="text-amber-600" />;
  ...
  const kpis = [
    { name: 'Total Employees', value: metrics?.totalEmployees, icon: Users, color: 'text-sky-500', bg: 'kpi-amber' },
    { name: 'Total HRs', value: metrics?.totalHRs, icon: UserCheck, color: 'text-emerald-500', bg: 'kpi-green' },
    { name: 'Total Managers', value: metrics?.totalManagers, icon: Users2, color: 'text-purple-500', bg: 'kpi-amber' },
    { name: 'Total Departments', value: metrics?.totalDepartments, icon: Network, color: 'text-amber-500', bg: 'kpi-green' },
    { name: 'Attendance Today', value: `${metrics?.attendanceToday}%`, icon: Clock, color: 'text-green-500', bg: 'kpi-green' },
    { name: 'Pending Leaves', value: metrics?.pendingLeaveRequests, icon: CalendarDays, color: 'text-rose-500', bg: 'kpi-amber' },
    { name: 'Payroll Processed', value: `${metrics?.payrollProcessed}%`, icon: CreditCard, color: 'text-indigo-500', bg: 'kpi-green' },
    { name: 'Active Users', value: metrics?.activeUsers, icon: Activity, color: 'text-amber-700', bg: 'kpi-amber' }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Title */}
      <div>
        <h2 className="text-2xl font-extrabold font-display text-slate-850 dark:text-white leading-tight">Admin Control Panel</h2>
        <p className="text-xs text-slate-500">Overview of organization health, workforce statistics, and system audits.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <CardContent className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={employeeGrowthData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#ffffff', borderColor: '#e2e8f0', borderRadius: '6px', fontSize: 10 }} />
        ...
        <Card className="accent-border-lavender flex flex-col">
          <CardHeader>
            ...
            {isActivitiesLoading ? (
              ...
              activities.map((act) => (
                <div key={act.id} className="flex gap-3 text-xs items-start bg-slate-50/50 p-2.5 rounded border border-slate-100 dark:bg-slate-800/10 dark:border-slate-800 animate-fadeIn">
                  ...
                  <div>
                    <p className="text-[11px] text-slate-700 font-bold dark:text-slate-300 leading-tight">{act.description}</p>
              ...
              <div className="text-center py-12 text-slate-400 text-xs">No audit logs recorded today.</div>
            )}
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="accent-border-mint md:col-span-2">
          <CardHeader>
            ...
            <div className="w-1/2 space-y-1.5 pl-4 overflow-y-auto max-h-[14rem]">
              {deptDistributionData.map((d, index) => (
                <div key={d.name} className="flex justify-between items-center text-[10px] font-bold">
                  ...
                  <span className="text-slate-800 dark:text-white font-mono">{d.value} members</span>
                </div>
              ))}
            </div>
          </CardContent>
