...
export const AttendanceOverview: React.FC = () => {
  ...
  const deptAttendanceStats = depts.map((d: any, index: number) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Title */}
      <div>
        <h2 className="text-2xl font-extrabold font-display text-foreground leading-tight">Workforce Attendance Overview</h2>
        <p className="text-xs text-foreground/60">Monitor today's active check-in metrics, tracking records, and department statistics.</p>
      </div>

      {/* KPI Cards Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4 flex items-center justify-between border-l-4 border-l-primary shadow-sm">
          <div>
            <span className="text-[10px] font-bold text-foreground/50 uppercase tracking-wide block">Roster Tracked</span>
            <span className="text-2xl font-extrabold text-foreground mt-1.5 block leading-none">{isLogsLoading ? '...' : totalChecked} members</span>
          </div>
        ...
        <Card className="p-4 flex items-center justify-between border-l-4 border-l-emerald-500 shadow-sm">
          <div>
            ...
            <span className="text-2xl font-extrabold text-foreground mt-1.5 block leading-none">{isLogsLoading ? '...' : presentCount} checked-in</span>
          </div>
        ...
        <Card className="p-4 flex items-center justify-between border-l-4 border-l-destructive shadow-sm">
          <div>
            ...
            <span className="text-2xl font-extrabold text-foreground mt-1.5 block leading-none">{isLogsLoading ? '...' : absentCount} members</span>
          </div>
        ...
        <Card className="p-4 flex items-center justify-between border-l-4 border-l-primary shadow-sm">
          <div>
            <span className="text-[10px] font-bold text-foreground/50 uppercase tracking-wide block">Attendance Ratio</span>
            <span className="text-2xl font-extrabold text-foreground mt-1.5 block leading-none">{isLogsLoading ? '...' : `${attendanceRate}%`}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-t-4 border-t-primary shadow-sm">
          ...
          <CardContent className="space-y-4 max-h-[16.5rem] overflow-y-auto pr-1">
            {deptAttendanceStats.map((ds: any) => (
              <div key={ds.name} className="space-y-1.5">
                <div className="flex justify-between text-xs font-bold">
                  <span className="text-foreground/80">{ds.name} Unit</span>
                  <span className="font-mono text-foreground">{ds.rate}%</span>
                </div>
      ...
      <Card className="border-t-4 border-t-emerald-500 shadow-sm">
        ...
        <CardContent>
          {isLogsLoading ? (
            <div className="text-center py-12 text-foreground/50 text-xs">Loading shift records...</div>
          ) : logs.length > 0 ? (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {logs.map((log: any) => (
                  <TableRow key={log.id}>
                    ...
                    <TableCell className="font-mono text-foreground/60">{log.clockIn || '--:--'}</TableCell>
                    <TableCell className="font-mono text-foreground/60">{log.clockOut || '--:--'}</TableCell>
                    <TableCell>
                      <Badge 
                        variant={
            ...
            <div className="text-center py-12 text-foreground/50 text-xs">No attendance logs found for today.</div>
          )}
        </CardContent>
      </Card>
